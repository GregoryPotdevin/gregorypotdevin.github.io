...
ArmaTools.Resizer = function()
{
  ...
  var expandOptions = function(options){
    ...
    if (!options.minWidth) options.minWidth = 8;
    return options;
  }
  ...
  var setupDrag = function(elem, options, dragFn){
    ...
    return function(e){
      ...
      var drag = function(e){
        if (isFirstMove){
          if (options.onFirstMove) options.onFirstMove();
          isFirstMove = false;
        }
        dragFn(e, info);
      }
  ...
  var initDragLeft = function(elem, options) {
    return setupDrag(elem, options, function(e, info) {
      ...
      var newLeft = clamp((info.left + e.clientX - info.startX), options.cMin, info.left+info.startWidth-options.minWidth);
  ...
  var initDragRight = function(elem, options) {
    return setupDrag(elem, options, function(e, info) {
      var width = clamp((info.startWidth + e.clientX - info.startX), options.minWidth, options.cMax-info.left);
  ...
  var makeMovable = function(elem, options){
    ...
    if (!options.noClasses) elem.className = elem.className + ' movable';
  ...
  var makeResizable = function(elem, options){
    var options = expandOptions(options);
    if (!options.noClasses) elem.classList.add('resizable');

    var handleLeft;
    if (options.leftHandle){
      handleLeft = options.leftHandle;
    } else {
    ...
    if (options.rightHandle){
      handleRight = options.rightHandle;
    } else {
    ...
    return {
      elem: elem,
        ...
        if (!options.noClasses) elem.classList.remove('resizable');
        if (!options.leftHandle) elem.removeChild(handleLeft);
        if (!options.rightHandle) elem.removeChild(handleRight);
      }
    };
  }
